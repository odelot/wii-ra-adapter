...
/* v0.28.6 do_frame profiling split: time spent resolving pointer chains
 * (rc_client_update_memref_values) vs evaluating conditions (the achievement
 * loop). Read by the .ino LOG_FRAME. ESP32-only (esp_timer). */
#include "esp_timer.h"
...
/* v0.29.1 diag: per-range eval time (a = core-1/do_frame half, b = core-0
 * worker half) + a run counter. If b stays 0 the worker never fires; if
 * a≈b but evl_us≈a+b they serialized; if a>>b or b>>a the split is skewed. */
...
/* v0.29.0 — parallel achievement evaluation. The do_frame's condition eval
 * (~75% of frame time) is split across both cores: this core takes the first
 * half of a subset's achievements, a worker pinned to Core 0 takes the second.
 * SAFE because the eval is read-only on the shared memrefs (every memref write
 * happens earlier, in the sequential rc_client_update_memref_values; the
 * Remember/AddAddress work is done there as modified_memrefs and is a no-op
 * during eval, and rc_update_trigger_memrefs is a no-op with has_memrefs=false).
 * Trigger + condition state is per-trigger, so disjoint ranges never collide.
 * The only SHARED writes are progress_tracker and game/subset pending_events;
 * each range accumulates those into a private rc_par_result_t, merged after the
 * barrier. */
#ifndef RC_PARALLEL_EVAL
#define RC_PARALLEL_EVAL 1
#endif
/* Below this count the semaphore/scheduling overhead outweighs the split. */
#define RC_PAR_MIN_ACHIEVEMENTS 24

typedef struct {
  float best_progress;
  rc_client_achievement_info_t* best_achievement;
  uint8_t game_events;
  uint8_t subset_events;
} rc_par_result_t;

/* Evaluate achievements [achievement, stop). Shared-state side effects go into
 * *res (progress + game/subset event bits); per-achievement pending_events and
 * per-trigger state are written directly (no cross-range aliasing). */
#ifdef RC_DIRTY_EVAL
/* ===========================================================================
 * Dirty / incremental eval  (compile guard RC_DIRTY_EVAL; runtime A/B toggle)
 *
 * Streaming every achievement's conditions each frame is the eval bottleneck.
 * Skip a trigger this frame iff it is COLD (trigger->has_hits==0: nothing true,
 * no leftover hits -> increments nothing, so a skip can't change current_hits/
 * measured/state) AND CLEAN (no memref it reads changed this OR last frame;
 * eval_next covers the Delta/Prior 1-frame settle) AND not WAITING. WARM triggers
 * evaluate every frame exactly as before. dep_memrefs==NULL => never skip.
 *
 * This is the optimization rcheevos itself hasn't shipped: the naive "skip if
 * memory unchanged" is WRONG (hitcounts accrue over time; Delta shifts) — the
 * cold+clean gate is what makes it provably equivalent. Validated host-side on
 * the upstream test suite: 2313/2314 with 92% of evals skipped, bit-identical.
 *
 * Per-achievement state (eval_next) is disjoint across the 2 work-stealing cores
 * (each achievement is owned by one core), so no race. The skip/eval counters are
 * global diagnostics -> atomic-relaxed to survive the 2-core increment.
 * =========================================================================== */
...
static void rc_de_add(rc_memref_value_t** arr, uint16_t* n, rc_memref_value_t* mv) {
  uint16_t i;
  if (!mv)
    return;
  for (i = 0; i < *n; ++i) {
...
static void rc_de_collect_operand(const rc_operand_t* op, rc_memref_value_t** arr, uint16_t* n) {
  /* a modified_memref (AddAddress/AddSource pointer chain) exposes its OWN resolved
   * value here, whose .changed is set in rc_client_update_memref_values — one entry
   * covers the whole chain. */
  if (rc_operand_is_memref(op))
    rc_de_add(arr, n, &op->value.memref->value);
}

static void rc_de_collect_condset(const rc_condset_t* cs, rc_memref_value_t** arr, uint16_t* n) {
  const rc_condition_t* c;
  if (!cs)
    return;
  for (c = cs->conditions; c != NULL; c = c->next) {
    rc_de_collect_operand(&c->operand1, arr, n);
    rc_de_collect_operand(&c->operand2, arr, n);
  }
}

static void rc_de_build_achievement(rc_client_game_info_t* game, rc_client_achievement_info_t* ach) {
  rc_memref_value_t* tmp[RC_DE_MAX_DEPS];
  uint16_t n = 0;
  const rc_trigger_t* trig = ach->trigger;
  const rc_condset_t* cs;

  ach->dep_memrefs = NULL;
  ach->dep_count = 0;
  ach->eval_next = 0;
  ...
  rc_de_collect_condset(trig->requirement, tmp, &n);
  for (cs = trig->alternative; cs != NULL; cs = cs->next)
    rc_de_collect_condset(cs, tmp, &n);

  if (n == 0 || n >= RC_DE_MAX_DEPS)   /* all-const, or hit the cap -> never skip */
    return;

  ach->dep_memrefs = (rc_memref_value_t**)rc_buffer_alloc(&game->buffer, n * sizeof(rc_memref_value_t*));
  if (ach->dep_memrefs) {
    memcpy(ach->dep_memrefs, tmp, n * sizeof(rc_memref_value_t*));
    ach->dep_count = n;
  }
}
...
/* 1 = skip this frame (guaranteed no-op), 0 = must evaluate. ASSERT: ACTIVE. */
static int rc_de_should_skip(rc_client_achievement_info_t* ach) {
  const rc_trigger_t* trig = ach->trigger;
  uint16_t i;
  ...
  for (i = 0; i < ach->dep_count; ++i) {
    if (ach->dep_memrefs[i]->changed) {
      dep_changed_now = 1;
      break;
    }
  }
...
static void rc_client_eval_achievement_range(rc_client_t* client,
  rc_client_achievement_info_t* achievement, rc_client_achievement_info_t* stop, int step, rc_par_result_t* res)
{
  for (; achievement < stop; achievement += step) {
    rc_trigger_t* trigger = achievement->trigger;
    ...
    if (trigger->measured_value != old_measured_value && old_measured_value != RC_MEASURED_UNKNOWN &&
      ...
      if (progress > res->best_progress) {
        res->best_progress = progress;
        res->best_achievement = achievement;
        res->game_events |= RC_CLIENT_GAME_PENDING_EVENT_PROGRESS_TRACKER;
        res->subset_events |= RC_CLIENT_SUBSET_PENDING_EVENT_ACHIEVEMENT;
 ...
 * runs, so no other thread touches the client concurrently. */
static struct {
  rc_client_t* client;
  rc_client_achievement_info_t* base;
  uint32_t n;
  rc_par_result_t result;
} g_par_job;
static volatile uint32_t g_par_next = 0;   /* work-stealing chunk cursor (shared, atomic) */
#define RC_PAR_CHUNK 4                      /* achievements per steal — bounds last-chunk imbalance vs atomic overhead */
static SemaphoreHandle_t g_par_start = NULL;
static SemaphoreHandle_t g_par_done = NULL;
static TaskHandle_t g_par_task = NULL;

static void rc_par_worker(void* arg) {
  (void)arg;
  for (;;) {
    int64_t _b0;
    uint32_t c, n;
     ...
     * core naturally pulls more — auto-balances regardless of where the heavy
     * achievements sit or how often the EXI task preempts core 1. */
    while ((c = (uint32_t)__atomic_fetch_add(&g_par_next, RC_PAR_CHUNK, __ATOMIC_RELAXED)) < n) {
      uint32_t end = c + RC_PAR_CHUNK; if (end > n) end = n;
      rc_client_eval_achievement_range(client, base + c, base + end, 1, &g_par_job.result);
    }
    g_par_b_us = (uint32_t)(esp_timer_get_time() - _b0);
    ++g_par_runs;
    xSemaphoreGive(g_par_done);
  }
}

/* Lazy init; returns 1 if the worker is ready. do_frame runs on core 1 (EXI),
 * httpTask idles on core 0 — pin the worker to core 0. 8KB stack covers
 * rc_test_condset's locals (no deep recursion; modified_memrefs pre-resolved). */
static int rc_par_init(void) {
  if (g_par_task) return 1;
  g_par_start = xSemaphoreCreateBinary();
  g_par_done  = xSemaphoreCreateBinary();
  if (!g_par_start || !g_par_done) return 0;
  if (xTaskCreatePinnedToCore(rc_par_worker, "rceval", 8192, NULL, 5, &g_par_task, 0) != pdPASS) {
    g_par_task = NULL;
    return 0;
  }
  return 1;
...
{
  rc_client_achievement_info_t* base = subset->achievements;
  uint32_t n = subset->public_.num_achievements;
  rc_par_result_t a;

  memset(&a, 0, sizeof(a));

#if RC_PARALLEL_EVAL
  if (n >= RC_PAR_MIN_ACHIEVEMENTS && rc_par_init()) {
    rc_par_result_t* b = &g_par_job.result;

    /* v0.32.2 WORK-STEALING split. The old stride-2 interleave gave each core
     * exactly half the achievements, but core 1 (the do_frame thread, sharing
     * its core with the prio-19 EXI task) runs ~1.25x slower than the core-0
     * worker — so par_a consistently overshot par_b (12.1 vs 9.7ms = 2.4ms
     * wasted at the barrier). Now both cores pull RC_PAR_CHUNK-sized chunks
     * from a shared atomic cursor until exhausted: the faster core pulls more,
     * auto-balancing to ~(par_a+par_b)/2 regardless of interrupt load or where
     * the heavy achievements cluster. g_par_next is stored 0 before the worker
     * is released, so the store happens-before the worker's first fetch_add. */
    ...
    { int64_t _a0 = esp_timer_get_time();
      uint32_t c;
      while ((c = (uint32_t)__atomic_fetch_add(&g_par_next, RC_PAR_CHUNK, __ATOMIC_RELAXED)) < n) {
        uint32_t end = c + RC_PAR_CHUNK; if (end > n) end = n;
        rc_client_eval_achievement_range(client, base + c, base + end, 1, &a);
      }
      g_par_a_us = (uint32_t)(esp_timer_get_time() - _a0); }

    /* barrier + merge the two private results */
    xSemaphoreTake(g_par_done, portMAX_DELAY);
    if (b->best_progress > a.best_progress) {
      a.best_progress = b->best_progress;
      a.best_achievement = b->best_achievement;
    }
    a.game_events   |= b->game_events;
    a.subset_events |= b->subset_events;
  }
  else
#endif
  {
    rc_client_eval_achievement_range(client, base, base + n, 1, &a);
  }

  /* apply the merged shared state */
  if (a.best_achievement && a.best_progress > client->game->progress_tracker.progress) {
    client->game->progress_tracker.progress = a.best_progress;
    client->game->progress_tracker.achievement = a.best_achievement;
  }
  client->game->pending_events |= a.game_events;
  subset->pending_events |= a.subset_events;
}
...
/* === GameCube/Wii RA adapter: cache-primed do_frame gate ===
 * (resolver/evaluator divergence cure, 2026-06-14)
 *
 * The adapter resolves pointer-chain leaf addresses across EXI round-trips
 * BEFORE evaluating conditions. The old predictor that drove resolution
 * (rc_memrefs_get_pending_addresses -> rc_resolve_cached_raw) is a hand-written
 * REPLICA of the evaluator's chain-walk (rc_get_modified_memref_value). The two
 * can diverge on exotic chain shapes: the predictor reports a set of addresses
 * the evaluator never reads (and misses ones it does), so the evaluator's read
 * hits an uncached address -> 0/tombstone -> a Delta-based "value became X"
 * condition fires falsely (e.g. SMG "Prismatic Plumber").
 *
 * Cure: stop predicting, start measuring. rc_client_do_frame's UPDATE pass IS
 * the exact read path the eval will use. We run it, then GATE on a callback
 * that reports whether every read hit the cache (the adapter's peek bumps a
 * miss counter on a no-tombstone read). If not primed, we ROLL BACK the memref
 * values (zero trace) and DEFER: no eval runs, so no condition ever sees a
 * transient 0. The misses recorded during the update drive the next
 * convergence round; the frame is evaluated once the cache is primed.
 * Resolved-set == read-set BY CONSTRUCTION — the replica's divergence can no
 * longer cause a false unlock. */
int  (*g_rc_doframe_primed_cb)(void) = NULL;   /* returns 0 -> cache not primed -> defer */
rc_memref_value_t* g_rc_doframe_save_buf = NULL;
uint32_t g_rc_doframe_save_cap = 0;            /* capacity (entries) of save_buf */
volatile int g_rc_doframe_gate_enabled = 1;    /* adapter clears it for a fail-open frame */
volatile int g_rc_doframe_deferred = 0;        /* set every gated do_frame: 1=deferred 0=evaluated */

/* Total base + modified memref count — sizes the adapter's rollback buffer.
 * Mirrors the traversal in rc_client_update_memref_values exactly. */
uint32_t rc_client_memref_count(const rc_client_t* client) {
  const rc_memrefs_t* mr;
  const rc_memref_list_t* ml;
  const rc_modified_memref_list_t* mml;
  uint32_t n = 0;
  if (!client || !client->game || !client->game->runtime.memrefs)
    return 0;
  mr = client->game->runtime.memrefs;
  ml = &mr->memrefs;
...
/* Snapshot / restore every memref value struct, in the SAME order as
 * rc_client_update_memref_values, so a deferred frame's update leaves zero
 * trace on the Delta the eventual primed frame computes. Saves the full
 * rc_memref_value_t (value+prior+changed) so the restore is exact. */
static uint32_t rc_gc_save_memref_values(const rc_memrefs_t* mr,
                                         rc_memref_value_t* buf, uint32_t cap) {
  const rc_memref_list_t* ml = &mr->memrefs;
  const rc_modified_memref_list_t* mml;
  uint32_t k = 0; uint16_t i;
  do {
    for (i = 0; i < ml->count && k < cap; ++i) buf[k++] = ml->items[i].value;
    ml = ml->next;
  } while (ml);
  mml = &mr->modified_memrefs;
  if (mml->count) {
    do {
      for (i = 0; i < mml->count && k < cap; ++i) buf[k++] = mml->items[i].memref.value;
      mml = mml->next;
    } while (mml);
  }
  return k;
...
static void rc_gc_restore_memref_values(rc_memrefs_t* mr,
                                        const rc_memref_value_t* buf, uint32_t count) {
  rc_memref_list_t* ml = &mr->memrefs;
  rc_modified_memref_list_t* mml;
  uint32_t k = 0; uint16_t i;
  do {
    for (i = 0; i < ml->count && k < count; ++i) ml->items[i].value = buf[k++];
    ml = ml->next;
  } while (ml);
  mml = &mr->modified_memrefs;
  if (mml->count) {
    do {
      for (i = 0; i < mml->count && k < count; ++i) mml->items[i].memref.value = buf[k++];
      mml = mml->next;
    } while (mml);
  }
}
...
void rc_client_do_frame(rc_client_t* client)
{
  ...
  g_rc_doframe_deferred = 0;   /* cache-primed gate: assume evaluated unless deferred below */
  ...
  if (client->game && !client->game->waiting_for_reset) {
    ...
    rc_client_subset_info_t* subset;
    int64_t _prof_t1;
    uint32_t _gc_saved = 0;

    rc_mutex_lock(&client->state.mutex);

    rc_client_reset_pending_events(client);

    /* Gate part 1 — snapshot memref values so a deferred frame can roll back
     * cleanly (zero trace on the Delta the eventual primed frame computes). */
    if (g_rc_doframe_gate_enabled && g_rc_doframe_primed_cb && g_rc_doframe_save_buf)
      _gc_saved = rc_gc_save_memref_values(client->game->runtime.memrefs,
                                           g_rc_doframe_save_buf, g_rc_doframe_save_cap);

    { int64_t _prof_t0 = esp_timer_get_time();
      rc_client_update_memref_values(client);   /* resolve pointer chains (1330 modified_memrefs) */
      _prof_t1 = esp_timer_get_time();
      g_rc_update_us = (uint32_t)(_prof_t1 - _prof_t0); }

    /* Gate part 2 — the update just read EXACTLY what the eval will read. If a
     * read missed the cache with no tombstone (would return 0), roll back and
     * defer: no condition is evaluated, so none sees a transient 0 (no false
     * "value became X"). The missed addresses were recorded by the adapter's
     * peek and drive the next convergence round; the frame is evaluated once
     * the cache is primed. See the comment above rc_client_memref_count. */
    if (g_rc_doframe_gate_enabled && g_rc_doframe_primed_cb && g_rc_doframe_save_buf
        && !g_rc_doframe_primed_cb()) {
      rc_gc_restore_memref_values(client->game->runtime.memrefs,
                                  g_rc_doframe_save_buf, _gc_saved);
#ifdef RC_INCREMENTAL_UPD
      /* This update was rolled back — its chain values did NOT commit. Clear the
       * opt-B warm flag so the next NON-deferred frame does a full resolve before
       * any skipping (else a chain whose first resolve was rolled back could be
       * B1-skipped while still holding its initial value). */
      client->game->runtime.memrefs->memrefs.incr_warmed = 0;
#endif
      g_rc_eval_us = 0;
      g_rc_doframe_deferred = 1;
      rc_mutex_unlock(&client->state.mutex);
      return;
    }

#ifdef RC_DIRTY_EVAL
    if (!client->game->dirty_eval_built)
      rc_de_build_game(client->game);   /* one-time, single-threaded, before the parallel eval */
#endif
     ...
     * this is essentially rc_test_condset over ~5896 comparison conditions. */
    g_rc_eval_us = (uint32_t)(esp_timer_get_time() - _prof_t1);
...
/* ===========================================================================
 * rc_client_get_memrefs - Prefetch API for external memory adapters
 *
 * Returns the rc_memrefs_t* associated with the loaded game so that the
 * caller can use rc_memrefs_get_addresses() to discover which addresses will
 * be read on the next rc_client_do_frame() call.
 *
