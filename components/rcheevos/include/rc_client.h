...
/**
 * Returns the rc_memrefs_t* for the currently loaded game, or NULL if no game is loaded.
 * Use with rc_memrefs_get_addresses() to collect all memory addresses that will be read
 * on the next rc_client_do_frame() call (including resolved AddAddress/Indirect reads).
 * This is useful for adapters that pre-fetch memory from a remote source (e.g. GameCube EXI).
 */
struct rc_memrefs_t;
RC_EXPORT const struct rc_memrefs_t* RC_CCONV rc_client_get_memrefs(const rc_client_t* client);
