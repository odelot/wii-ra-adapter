...
#ifdef RC_SHORT_CIRCUIT
/* Optimization C runtime A/B toggle (see rc_evaluate_trigger). Default ON. */
...
#ifdef RC_SHORT_CIRCUIT
  /* Optimization C: enable the dormant short-circuit so a condset stops once its
   * result is decided. LOGIC-NEUTRAL for unlocks: hit-TARGET and Measured groups
   * are always evaluated (rc_test_condset passes 0 there); only momentary "other"
   * conditions (required_hits==0) get skipped, and their current_hits is a cosmetic
   * tally that never feeds cond_valid. Host suite: 4 cosmetic hit-count/has_hits
   * asserts diverge; test_client (unlock logic) stays clean. SYNERGY with dirty-
