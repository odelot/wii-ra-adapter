...
typedef struct rc_modified_memref_t {
  ...
  /* opt: reverse-hash incremental collect. 1 = this INDIRECT chain's inputs were
   * unchanged since its last walk, so rc_memrefs_get_pending_addresses can SKIP
   * re-walking it. Lives in the existing padding between modifier_type and depth
   * -> sizeof(rc_modified_memref_t) is unchanged (the RC_ALLOC trap, see B1). */
  uint8_t incr_clean;
#endif
...
typedef struct rc_memref_list_t {
  ...
  /* opt B: 0 until the first full rc_update_memref_values resolves every chain;
   * incremental skipping starts on the 2nd pass. Stored here (in the head list's
   * existing padding after 'allocated') rather than on rc_memrefs_t so it does
   * NOT change sizeof(rc_memrefs_t) -- growing that struct breaks rcheevos's
   * two-pass parse-buffer size calc (heap overflow on richpresence). */
  uint8_t incr_warmed;
#endif
} rc_memref_list_t;
...
#ifdef RC_INCREMENTAL_UPD
/* opt B: shared skip predicate for the modified_memref update loop (used by both
 * rc_update_memref_values and rc_client.c's rc_client_update_memref_values). */
int rc_modified_memref_can_skip(rc_modified_memref_t* mm, int incr);
/* opt B state/instrumentation (defined in memref.c). The adapter sets the hook
 * + toggle and reads the per-frame counters for its FRAME log. */
 ...
 * already present in the caller's cache (so peek would return a real value
 * for them). Used by rc_memrefs_get_pending_addresses to know how deep a
 * pointer chain can be resolved from cached data alone. */
 ...
 * chains LEVEL BY LEVEL using the caller's cache: each chain is walked from
 * its static root through peek, and the FIRST address along the path that is
 * not yet cached (is_cached==0) is reported as "pending". Walking stops at
 * that point — it does NOT cascade past an unresolved pointer into garbage
 * addresses. Call it in a loop: fetch the reported addresses, add them to the
 * cache, call again; each round exposes the next chain level. Convergence is
 * when it returns 0 (every leaf reachable from cache).
 *
 * Unlike rc_memrefs_get_addresses, this uses parent->VALUE resolved through
 * the cache (not parent->address from the last do_frame), so a single frame
 * can resolve an entire multi-level chain. READ-ONLY: never writes any
 * memref value/prior/address, so deltas are preserved for the next do_frame.
 *
 * Returns the number of (deduplicated) pending addresses written. */
 ...
 * records, during a chain walk, the (address -> chain) pairs into its reverse
 * index; then per snapshot it marks only the chains reading a CHANGED address as
 * dirty, and rc_memrefs_get_pending_addresses skips the rest.
 *   g_rc_chain_read_cb : called for every address a chain reads while walking;
 *                        `chain` is the rc_modified_memref_t* (opaque to caller).
 *   g_rc_incr_collect_enabled : master A/B toggle (0 => behave exactly as before).
 *   rc_modified_memref_mark_dirty : adapter marks one chain to be re-walked.
 *   rc_modified_memrefs_mark_all_dirty : force a full rebuild (all chains walk). */
