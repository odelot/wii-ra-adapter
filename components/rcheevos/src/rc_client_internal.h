...
#ifdef RC_DIRTY_EVAL
  /* dirty/incremental eval: distinct rc_memref_value_t this achievement reads.
   * NULL => "never skip" (no deps / overflow / alloc-fail -> always evaluate). */
  rc_memref_value_t** dep_memrefs;
  uint16_t dep_count;
  uint8_t eval_next;   /* 1 => force one more eval next frame (Delta/Prior settle) */
#endif
